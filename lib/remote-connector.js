...
// ObjectId instances expose toHexString(); other objects (Date, Buffer, URL,
// plain objects) do not. Coercing only ObjectIds keeps the patch narrow — it
// preserves strong-remoting's strict typing for every other value.
function coerceObjectIdArgs(args, accepts) {
  if (!accepts || !args.length) return args;
  return args.map(function(val, i) {
    const accept = accepts[i];
    if (!accept || accept.type !== 'string') return val;
...
function createProxyMethod(Model, remotes, remoteMethod) {
  ...
  function remoteMethodProxy() {
    ...
    args = coerceObjectIdArgs(args, remoteMethod.accepts);

    if (remoteMethod.isStatic) {
      remotes.invoke(remoteMethod.stringName, args, callback);
    } else {
      let ctorArgs = [this.id];
      const ctorAccepts = remoteMethod.restClass &&
        remoteMethod.restClass.ctor && remoteMethod.restClass.ctor.accepts;
      ctorArgs = coerceObjectIdArgs(ctorArgs, ctorAccepts);
